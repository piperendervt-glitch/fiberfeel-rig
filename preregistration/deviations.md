...
## 2026-05-03 17:59: §4 ExposureTime_us コメント解釈の確定
- 逸脱内容：プレレジからの逸脱ではなく、§4 コメント「ROI 輝度がフルスケールの 30-70%」の「ROI 輝度」を ROI 内 per-pixel max と解釈したことを記録する。プレレジ §5.1 / §9 の解析単位は ROI sum だが、§4 コメントの「フルスケールの %」表現は per-pixel ADC 値 (4095) に対してのみ意味を持つため
