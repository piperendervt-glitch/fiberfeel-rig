...
//   z =  5 .. 13   Ø5.1  LED 本体穴（LED 砲弾 8mm を完全収納）
//   z = 13 .. 14   実体  LED 上面と POF 端面を隔てる 1mm の壁
//   z = 14 .. 24   Ø1.8  POF 縦穴
// ============================================
...
// POF 縦穴の z 開始位置（壁の上）
pof_z0           = led_body_z0 + led_body_depth_mm + led_pof_wall_t;  // = 14

// M3 ナットトラップ（POF セクション中央付近）
nut_trap_z         = pof_z0 + pof_side_l / 2;              // = 19
...
module led_fiber_coupler() {
    difference() {
        ...
        // --- 側面の光モニタ穴（任意）---
        // LED 上面と POF 端面の間の壁（z=13..14）を貫く位置で、両側を覗ける。
        // 直径方向に貫通。X 軸に沿わせる。
        translate([0, 0, led_body_z0 + led_body_depth_mm + led_pof_wall_t / 2])  // = 13.5
            rotate([0, 90, 0])
